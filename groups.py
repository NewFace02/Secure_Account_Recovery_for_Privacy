...
try:
    # 这里引入你需要的曲线实现
    from curve.suite_p256 import p256_sswu_ro, p256_order, p256_p, p256_F, p256_A, p256_B
...
# GroupNISTCurve 是对 NIST 椭圆曲线的封装
class GroupNISTCurve(Group):
    def __init__(self, name, suite, F, A, B, p, order, gx, gy, L, H, expander, k):
        ...
        self.field_bytes_length = int(math.ceil(len(self.p.bits()) / 8))
   ...
   # this is using point compression
    ...
    def element_byte_length(self):
        return int(_sage_const_1  + self.field_bytes_length)
    ...
    def hash_to_scalar(self, msg, dst):
        ...
        return hash_to_field(msg, _sage_const_1 , self.order(), self.m, self.L, expander)[_sage_const_0 ][_sage_const_0 ]
...
class GroupP521(GroupNISTCurve):
    def __init__(self):
        ...
        GroupNISTCurve.__init__(self, "P521_XMD:SHA-512_SSWU_RO_", p521_sswu_ro, p521_F, p521_A, p521_B, p521_p, p521_order, gx, gy, _sage_const_98 , hashlib.sha512, XMDExpander, _sage_const_256 )
    def hash_to_group(self, msg, dst):
...
class GroupRistretto255(Group):
    def __init__(self):
        ...
        self.k = _sage_const_128 
        self.L = _sage_const_48 
        self.field_bytes_length = _sage_const_32 
    ...
    def serialize_scalar(self, scalar):
        return I2OSP(scalar % self.order(), self.scalar_byte_length())[::-_sage_const_1 ]
    ...
    def hash_to_scalar(self, msg, dst):
        uniform_bytes = expand_message_xmd(msg, dst, _sage_const_64 , hashlib.sha512, self.k)
...
class GroupDecaf448(Group):
    def __init__(self):
        ...
        self.k = _sage_const_224 
        self.L = _sage_const_84 
        self.field_bytes_length = _sage_const_56 
    ...
    def hash_to_scalar(self, msg, dst):
        uniform_bytes = expand_message_xof(msg, dst, int(_sage_const_64 ), hashlib.shake_256, self.k)
        return OS2IP_le(uniform_bytes) % self.order()
